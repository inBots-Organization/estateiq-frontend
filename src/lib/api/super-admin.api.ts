 ...
 *
 * API client for super admin operations including:
 * - Platform overview and dashboard
 * - Organization management
 * - Subscription and plan management
 * - Revenue and API usage analytics
...
export const superAdminApi = {
  ...
  getOrganizations: (params?: {
    ...
    return fetchApi<PaginatedResult<OrganizationSummary>>(
      `/organizations${query ? `?${query}` : ''}`
    );
  },

  getOrganization: (id: string) =>
    fetchApi<OrganizationDetails>(`/organizations/${id}`),
  ...
  // Plans
  getPlans: () => fetchApi<SubscriptionPlan[]>('/plans'),
  ...
  getUserDetails: (id: string) => fetchApi<UserSummary>(`/users/${id}`),
  ...
  getAuditLogs: (params?: {
    ...
    const query = searchParams.toString();
    return fetchApi<PaginatedResult<AuditLogEntry>>(
      `/audit-logs${query ? `?${query}` : ''}`
    );
  },

  // System
  getSystemHealth: () => fetchApi<{ status: string; timestamp: string }>('/health'),
};
